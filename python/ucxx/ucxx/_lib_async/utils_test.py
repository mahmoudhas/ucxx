...
def compute_timeouts(pytestconfig: pytest.Config) -> tuple[float, float]:
    ...
    timeout, high timeout and finally test timeout.

    Parameters
    ----------
    pytestconfig : pytestconfig
        The pytestconfig object retrieved by the fixture with the same name.
    ...
    """
    plugin_timeout = pytestconfig.cache.get("trio_test_timeout", {}).get("timeout", 60)
